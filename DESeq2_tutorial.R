...
#2-4
## Write the result 
out_mat = cbind(counts(dds_mat,normalized=T),res_mat) %>% data.frame
...
#write.xlsx(tx2gene,'tx2gene.xlsx')
...
colnames(txi$counts)=metadata_txi$sample
head(txi$counts)
...
dim(dds_mat)
## specify the reference level
dds_mat$Condition = relevel(dds_mat$Condition,ref = 'KEAP1_lof')
dds_mat$Condition
...
## save the data
out_mat = cbind(counts(dds_mat,normalized=T),res_mat) %>% data.frame
write.xlsx(out_mat,'DESeq2 result from salmon result.xlsx',rowNames=T)
